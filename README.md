...
| `OPENAI_BASE_URL`   | Optional OpenAI-compatible API root (for example LM Studio at `http://127.0.0.1:1234/v1`)                |
...
For AI, open **Settings → AI**, select **Anthropic** or **OpenAI**, and save. Cloud providers need an API key; OpenAI also accepts an OpenAI-compatible API base URL for local servers such as LM Studio (`http://127.0.0.1:1234/v1`) without a cloud key. OpenAI defaults to `gpt-4.1-mini`; for LM Studio, paste the model ID shown on the loaded model. Each provider keeps its own encrypted key and model choice. Existing Anthropic settings continue to work. An OpenAI-only environment setup selects OpenAI automatically; with both keys present, Anthropic remains the default until you save a provider choice. Environment keys and `OPENAI_BASE_URL` override saved values and must be changed on the server. Saving settings does not make a model request or verify account access.
...
flowchart LR
    ...
    IM["15 documented statement formats + CSV mapper"] --> EX
    MN["Manual entry"] --> EX
...
| **Statement import**     | **15 documented formats**, auto-detected: **TradeZella** and **Tradervue** (CSV migration), TradingView (paper and strategy exports), MetaTrader 4 and 5, ThinkorSwim, IBKR (activity CSV, Flex CSV, and Flex XML), NinjaTrader, Tradovate, TopstepX, Webull, DAS Trader, plus a column mapper for any other CSV. Matching execution records dedupe within an account; review warnings and skipped rows before committing an import. Have an export we don't recognize? Open an issue with an anonymized sample; real files are the most useful contribution this repo can get. |
| **Round-trip engine**    | Flat-to-flat position cycles from raw fills. FIFO / LIFO / weighted-average per account. Partial fills, scale-ins, flips, futures multipliers, equity-option contracts (default 100). Annotations survive rebuilds.                                                                                                                                                                                                                                                                                                                                                             |
...
| **AI reflection**        | Bring your own Anthropic or OpenAI API key, or an OpenAI-compatible local server such as LM Studio: session recaps, per-trade critiques, "ask your journal" over your own aggregates. Cloud keys encrypted at rest; requests go from your server to the model, nowhere else.                                                                                                                                                                                                                                                                                                    |
...
| **Journal defaults**     | Configure breakeven tolerance, plus fee and stop/target rules per account and symbol; set timezone and contract multipliers (equity options default to 100) for consistent calculations.                                                                                                                                                                                                                                                                                                                                                                                        |
...
**Interactive Brokers Flex Query:** connect a sync account or upload the Flex XML on **Import → File upload**. Include Strike, Expiry, Put/Call, Multiplier, Cost Basis, and Realized P/L so option contracts stay separate from stock and expirations/assignments journal at IBKR's settlement value rather than a $0 BookTrade price. Overlapping XML files dedupe on IBKR transaction IDs. See [Interactive Brokers Flex Query](docs/importers.md#interactive-brokers-flex-query).
